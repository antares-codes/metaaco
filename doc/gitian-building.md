...
*Setup instructions for a gitian build of MetaACO Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MetaACO
...
to the MetaACO GitHub release page.
...
- [Building MetaACO Core](#building-metaaco-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/metaaco-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/metaaco-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/metaaco-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/metaaco-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/metaaco-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/metaaco-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/metaaco/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@antares.codes) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/metaaco-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/metaaco-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/metaaco-osx-*-build.assert
```
