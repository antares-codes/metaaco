...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::MACO);
    ...
    if (settings.contains("nAnonymizeMetaACOAmount") || reset)
        SoftSetArg("-anonymizemetaacoamount", settings.value("nAnonymizeMetaACOAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in metaaco.cpp during shotdown to also remove the window positions
