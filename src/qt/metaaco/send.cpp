...
#include "qt/metaaco/send.h"
#include "qt/metaaco/forms/ui_send.h"
#include "qt/metaaco/addnewcontactdialog.h"
#include "qt/metaaco/qtutils.h"
#include "qt/metaaco/sendchangeaddressdialog.h"
#include "qt/metaaco/optionbutton.h"
#include "qt/metaaco/sendconfirmdialog.h"
#include "qt/metaaco/myaddressrow.h"
#include "qt/metaaco/guitransactionsutils.h"
...
SendWidget::SendWidget(MetaACOGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (MACO)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("MetaACO address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 MACO");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_MACO : AskPassphraseDialog::Context::Send_zMACO, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zMACO is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zMACO selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zMACO transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZMACO_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zMACO require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zMACO transaction
        ...
        emit message("zMACO transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isMACO){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zMACO in your balance to select."));
        }
    }
}
...
void SendWidget::onMACOSelected(bool _isMACO){
    isMACO = _isMACO;
    setCssProperty(coinIcon, _isMACO ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
