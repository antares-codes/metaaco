...
#include "qt/metaaco/privacywidget.h"
#include "qt/metaaco/forms/ui_privacywidget.h"
#include "qt/metaaco/qtutils.h"
#include "guiutil.h"
#include "qt/metaaco/denomgenerationdialog.h"
#include "qt/metaaco/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(MetaACOGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zMACO anonymizes your MACO by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zMACO or convert back to MACO"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 MACO ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zMACO");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zMACO");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zMACO");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zMACO");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zMACO");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zMACO");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zMACO");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zMACO");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zMACO Movements"));
    ...
    // Only Convert to MACO enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzMACO->setTitleClassAndText("btn-title-grey", "Total 0 zMACO");
    ui->btnTotalzMACO->setSubTitleClassAndText("text-subtitle", "Show denominations of zMACO owned.");
    ui->btnTotalzMACO->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select MACO outputs to mint into zMACO.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zMACO");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzMACO, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zMACO");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of MACO to mint into zMACO"));
    }else{
        btnText = tr("Convert back to MACO");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zMACO to convert back into MACO"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzMACO->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzMACO->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zMACO is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zMACO, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zMACO").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zMACO minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zMACO converted back to MACO"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any MACO to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMACO </b>";
    ...
    ui->btnTotalzMACO->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
