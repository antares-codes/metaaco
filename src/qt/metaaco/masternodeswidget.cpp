...
#include "qt/metaaco/forms/ui_masternodeswidget.h"
#include "qt/metaaco/qtutils.h"
#include "qt/metaaco/mnrow.h"
#include "qt/metaaco/mninfodialog.h"

#include "qt/metaaco/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(MetaACOGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= CollateralRequired(chainActive.Height())){
            ...
            // kind of like "...node, %i MACO requited").arg(qYourCollateralResultVariable),
            // to nicely print required collateral amount.
            inform(tr("Not enough balance to create a masternode")); //, 25,000 MACO required."));
            return;
        }
        showHideOp(true);
