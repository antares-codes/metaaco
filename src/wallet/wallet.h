...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MACO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMACO send
enum ZerocoinSpendStatus {
    ZMACO_SPEND_OKAY = 0,                            // No error
    ZMACO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMACO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMACO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMACO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMACO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMACO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMACO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMACO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMACO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMACO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMACO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMACO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMACO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMACO_SPENT_USED_ZMACO = 14,                      // Coin has already been spend
    ZMACO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMACO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZMACOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzMACOTracker> zpivTracker;
    ...
    void setZWallet(CzMACOWallet* zwallet);
    CzMACOWallet* getZWallet();
    ...
    /// Get 10000 MACO output and keys which can be used for the Masternode
    ...
    /** zMACO reset */
    boost::signals2::signal<void()> NotifyzMACOReset;
