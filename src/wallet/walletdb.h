...
public:
    ...
    bool WriteZMACOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZMACOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZMACOSeed_deprecated(uint256& seed);
    bool EraseZMACOSeed();
    bool EraseZMACOSeed_deprecated();

    bool WriteZMACOCount(const uint32_t& nCount);
    bool ReadZMACOCount(uint32_t& nCount);
