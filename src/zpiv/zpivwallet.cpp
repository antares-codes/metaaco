...
CzMACOWallet::CzMACOWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMACOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMACOSeed_deprecated()) {
                    LogPrintf("%s: Updated zMACO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzMACOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMACOCount(nCountLastUsed);
    else if (!walletdb.ReadZMACOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMACOWallet::Lock()
{
    seedMaster = 0;
}

void CzMACOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMACOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMACO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMACOWallet::LoadMintPoolFromDB()
{
...
void CzMACOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMACOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMACOWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzMACOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMACO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzMACOWallet::SeedToZMACO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMACOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMACOWallet::GenerateDeterministicZMACO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMACOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMACOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMACOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
