...
CzMACOTracker::CzMACOTracker(std::string strWalletFile)
{
...
CzMACOTracker::~CzMACOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMACOTracker::Init()
{
...
bool CzMACOTracker::Archive(CMintMeta& meta)
{
...
bool CzMACOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMACOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMACOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMACOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMACOTracker::GetSerialHashes()
{
...
CAmount CzMACOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMACOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMACOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMACOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMACOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMACOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMACOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMACOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMACOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMACOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMACOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMACOWallet* zMACOWallet)
{
    bool iszMACOWalletInitialized = (NULL != zMACOWallet);
    CMintMeta meta;
    ...
    if (! iszMACOWalletInitialized)
        zMACOWallet = new CzMACOWallet(strWalletFile);
    meta.isSeedCorrect = zMACOWallet->CheckSeed(dMint);
    if (! iszMACOWalletInitialized)
        delete zMACOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMACOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMACOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMACOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMACOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMACOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMACOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMACOWallet* zMACOWallet = new CzMACOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMACOWallet);
        }
        delete zMACOWallet;
